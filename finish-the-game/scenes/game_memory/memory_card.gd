...
func filp_card(is_up: bool) -> void:
	var tween: Tween = get_tree().create_tween()
	
	if is_up:
		tween.tween_property(self, "scale", Vector2.DOWN, 0.07)
		...
		tween.tween_property(self, "scale", Vector2.LEFT + Vector2.DOWN, 0.07)
		...
		tween = get_tree().create_tween()
		tween.tween_property(self, "scale", Vector2.RIGHT + Vector2.DOWN, 0.07)
