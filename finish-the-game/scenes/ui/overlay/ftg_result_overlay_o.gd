...
func _ready() -> void:
	...
	tween_increase_angle_offset\
	.tween_property(self, "angle_offset", 2*PI, increase_duration)\
	.set_trans(Tween.TRANS_SINE)
	await tween_increase_angle_offset.finished
	
	var tween_increase_start_angle: Tween = get_tree().create_tween()
	tween_increase_start_angle\
	.tween_property(self, "start_angle", start_angle_offset + 2*PI, decrease_duration)\
	.set_trans(Tween.TRANS_SINE)
	
	var tween_decrease_angle_offset: Tween = get_tree().create_tween()
	tween_decrease_angle_offset\
	.tween_property(self, "angle_offset", 0, decrease_duration)\
	.set_trans(Tween.TRANS_SINE)
